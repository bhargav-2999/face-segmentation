...
# ✅ Automatically download model weights if not present
def download_model_if_needed(weight_path='79999_iter.pth'):
    if not os.path.exists(weight_path):
        print("🔽 Downloading BiSeNet model weights...")
        gdown.download(
...
def load_bisenet_model(weight_path='79999_iter.pth', n_classes=19):
    download_model_if_needed(weight_path)  # ✅ auto-download if needed
    net = BiSeNet(n_classes=n_classes)
    net.load_state_dict(torch.load(weight_path, map_location='cpu'))
    net.eval()
    return net
...
def create_rgba_image(original_bgr, mask):
    ...
    rgba_image = np.zeros((orig_h, orig_w, 4), dtype=np.uint8)
    face_pixels = (mask == 255)
    rgba_image[face_pixels, 0:3] = original_rgb[face_pixels, :]
    rgba_image[face_pixels, 3] = 255
    return rgba_image
...
def make_glasses_see_through(rgba_image, left_eye_box, right_eye_box):
    return rgba_image  # Feature disabled for now
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python face_segment_fin.py <image_path>")
        sys.exit(1)
